...
const ArticleItem = ({ article }: { article: ArticleListProps['staticArticles'][0] }) => (
...
export const ArticleList: ComponentConfig<ArticleListProps> = {
  fields: {
    source: {
      type: "radio",
      options: [
        { label: "Dynamic (Supabase)", value: "dynamic" },
  ...
  defaultProps: {
    ...
    maxArticles: 5,
    showDivider: true,
    staticArticles: Array(5).fill({}).map((_, i) => ({
      id: `article-${i}`,
      number: i + 1,
      title: `Article ${i + 1}`,
      date: "12.06.2021",
  ...
  render: ({ source, maxArticles = 5, showDivider, staticArticles = [] }) => {
    const { data: dynamicArticles = [], isLoading } = useArticles(source === 'dynamic' ? maxArticles : 0)
    const articles = source === 'dynamic' ? dynamicArticles : staticArticles

    if (source === 'dynamic' && isLoading) {
