...
export const Socials: ComponentConfig<SocialsProps> = {
  ...
  render: ({ showSubscribe }) => {
    ...
    return (
            ...
            <p className={`text-adaptive-secondary text-base transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0'}`}>{"Looking for Substance?"}</p>
            <p><span className={`text-adaptive-secondary text-base italic line-through transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0'}`}>{"Subscribe"}</span></p>
            <p><span className={`italic text-base transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0'}`}>{"Prescribe to us"}</span></p>
            {showSubscribe && (
